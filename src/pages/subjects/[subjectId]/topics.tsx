...
const Topics: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTopics = async () => {
      setLoading(true);
      try {
        const response = await fetch(`http://localhost:4000/api/topics/${subjectId}/topics`);
        if (!response.ok) {
  ...
  const handleAddTopic = async (newTopic: FormData) => {
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:4000/api/topics/${subjectId}/addTopicWithUploader`, {
        method: 'POST',
